...
def process_gpt_output(output):
    ...
    work_experience = []
    work_exp_sections = re.findall(r'RELEVANT WORK EXPERIENCE:\s*(.*?)\n\n', output, re.DOTALL)
    if not work_exp_sections:
        work_exp_sections = re.findall(r'RELEVANT WORK EXPERIENCE:\s*(.*)', output, re.DOTALL)
    for exp in work_exp_sections:
        experiences = exp.strip().split('\n\n')
        work_experience.extend([experience.strip() for experience in experiences if experience.strip()])
    
    cover_letter_info_raw = re.sub(r'^COVER LETTER INFO:\s*', '', sections[-1], flags=re.MULTILINE).strip().split('\n')
    cover_letter_info = {item.split(':', 1)[0].strip(): item.split(':', 1)[1].strip() for item in cover_letter_info_raw if ':' in item}
...
"""
    ...
    for idx, experience in enumerate(work_experience, 1):
        full_resume += f"\n{idx}. {experience}\n"
...
def generate_cover_letter(resume, job_description, cover_letter_info):
    ...
    formatted_cover_letter = f"""{cover_letter_info.get('Full Name', '')}
...
def create_pdf(content, filename):
    ...
    if filename == "cover_letter.pdf":
        ...
        first_name = header_info.split()[0] if header_info else "Candidate"
